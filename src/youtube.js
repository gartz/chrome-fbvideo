...
(function () {
    ...
    function moveVideos() {
        ...
        if (!viewPorts.length) {
            fixedVideos = divSwfObjects.querySelectorAll('[data-position="fixed"]');

            Array.prototype.forEach.call(fixedVideos, function (element) {
                resetFixed(element);
            });

            if (fixedVideos.length) {
        ...
        // Recalculate only if some video change from Fixed to Relative
        if (fixedVideos.length !== divSwfObjects.querySelectorAll('[data-position="fixed"]').length) {
            minimizeVideos();
        }
    }
    ...
    function resizeVideo(element, options) {
        ...
        if (options.width && options.width !== element.clientWidth) {
            element.style.width = options.width + 'px';
            iframe.setAttribute('width', options.width);
        }

        if (options.height && options.height !== element.clientHeight) {
    ...
    function minimizeVideos() {
        ...
        var notResizeds = divSwfObjects.querySelectorAll('[data-notresize]');
        ...
        Array.prototype.forEach.call(videos, function (element) {
            ...
            //var areaHeight = divSwfObjects.getBoundingClientRect().height;

            // max height to be used
            var maxHeight = (window.innerHeight / 3) * 2;
            var swfObjLength = divSwfObjects.children.length;

            var dinamicSizeNum = swfObjLength - notResizeds.length;
            var height = (maxHeight - notResizedsSize) / (dinamicSizeNum);
